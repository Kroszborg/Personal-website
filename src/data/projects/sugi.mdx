...
- **Message-passing concurrency**: all async operations (git commands, API calls, AI requests) use Bubbletea's `tea.Cmd` → `tea.Msg` pipeline — the UI thread never blocks
...
- **Adaptive layout**: every `WindowSizeMsg` recalculates all panel dimensions — Sugi handles terminal resize correctly at all times
